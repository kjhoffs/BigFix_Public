...
This sample demonstrates performing downloads using Basic Authentication (Username and Password).
"""
# Standard HTTP Basic Authentication requires sending an 'Authorization: Basic <base64>' header, 
# where <base64> is the base64-encoded string of "username:password".
# Example curl using explicit basic auth headers (where dXNlcm5hbWU6cGFzc3dvcmQ= is 'username:password' in base64):
#curl --header "Authorization: Basic dXNlcm5hbWU6cGFzc3dvcmQ=" https://raw.githubusercontent.com/Jwalker107/BasicAuthDownloadPlugin/main/README.md
#curl -L -H "Accept: application/octet-stream" -H "Authorization: Basic dXNlcm5hbWU6cGFzc3dvcmQ=" https://api.github.com/repos/Jwalker107/BasicAuthDownloadPlugin/releases/assets/141569199 -O
...
def get_config(config_file:str) -> dict:
    ...
    # config.json stores options such as the download plugin name, log file name & log level, and optionally may have a 'password' value to store
    try:
...
def get_credential_identifier(config:dict, url_config:dict) -> str:
    """Return an identifier name for one url_config section of the config"""
    return f"{config.get('plugin_name', 'BasicAuthDownload')}_{url_config.get('config_name', 'UnNamed')}"

def update_credentials(config:dict, config_file:str) -> None:
    """Checks whether an updated password is present in the configuration file.  If so, update the keyring and remove the password from the file."""
    updates_found=False
    for url_config in config.get('url_configs', []):
        identifier=get_credential_identifier(config, url_config)
        
        # Pull username and password from config
        username = url_config.get('username', 'BasicAuthDownload')
        password = url_config.get('password', None)
        
        if password is not None:
            updates_found=True
            logging.info(f'Storing password to keyring for {identifier}')
            keyring.set_password(
                identifier,
                # storing under the configured username:
                username,
                password
            )
            url_config['password']=None
            logging.info(f'Removing password from config file for config {identifier}')
...
def set_config(config:dict, config_file:str) -> None:
    ...
    # If the config file contained a 'password' value, we will re-write the file to remove that value after storing it in the keyring
    with open(config_file, 'w') as file:
        json.dump(config, file, indent=2)


def get_password(identifier:str, username:str) -> str:
    logging.info(f"Retrieving keyring credential for {identifier}")
    token_container=keyring.get_credential(identifier, username)
...
def download_file_stream(
    ...
    with session.get(url, stream=True, allow_redirects=True, verify=False) as response:
        if not response.ok:
            raise ValueError(
...
def process_download_list(options:dict, config:dict, session:requests.Session) -> list[dict]:
    """
    Process the download requests provided in 'options' dictionary, using url configurations defined in 'config', with a reusable requests.Session.
    ...
    plugin_system_name = config.get('plugin_name', "BasicAuthDownload")
    results = []
    for download in options.get("downloads", []):
        ...
        # report a download error if a password could not be retrieved for the matched url_configs entry
        identifier=get_credential_identifier(config, url_config)
        username=url_config.get('username', 'BasicAuthDownload')
        password=get_password(identifier, username)
        
        if password is None:
            download_result={'id': download['id'], 'success': False, 'error': f'Failed to retrieve password for {identifier}, try adding password to config.json'}
            results.append(download_result)
            continue

        # Attempt to perform the download and report the actual download result.
        # Mirroring the Token Auth plugin behavior, explicitly construct and inject the Authorization header.
...
def main(downloads=None) -> None:
    ...
    # If 'password' has a value in any url_configs stanza in config.json, use keyring to encrypt the password and then remove it from the config file
    # Note - we want to update *all* passwords *anywhere* in the config, before attempting downloads, so we can be sure the plaintext
    # password is removed from the config file as soon as possible
